...
class GroupBackend extends \OC\Group\Backend {
	...
	public function getGroups($search = '', $limit = null, $offset = null) {
		...
		$stmt = \OC_DB::prepare('SELECT `gid` FROM `*PREFIX*fg_groups`' . $searchLike . ' ORDER BY `gid` ASC', $limit, $offset);
	...
	public function usersInGroup($gid, $search = '', $limit = null, $offset = null) {
		...
		$stmt = \OC_DB::prepare(
			'SELECT `uid` FROM `*PREFIX*fg_group_user` WHERE `gid` = ?' . $searchLike . ' ORDER BY `uid` ASC',
			$limit,
			$offset
		);
		$result = $stmt->execute($parameters);
	...
	public function countUsersInGroup($gid, $search = '') {
		...
		$stmt = \OC_DB::prepare('SELECT COUNT(`uid`) AS `count` FROM `*PREFIX*fg_group_user` WHERE `gid` = ?' . $searchLike);
		$result = $stmt->execute($parameters);
