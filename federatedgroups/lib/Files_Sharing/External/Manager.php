...
class Manager {
	...
	public function addShare($remote, $token, $password, $name, $owner, $accepted=false, $groupId = null, $remoteId = -1) {
		...
		if (!$accepted) {
			...
			$i = 1;
			while (!$this->connection->insertIfNotExist('*PREFIX*share_external_group', $data, ['user', 'mountpoint_hash'])) {
			...
			$groupRowId = $this->connection->lastInsertId('*PREFIX*share_external_group');
			$users = \OC::$server->getGroupManager()->findUsersInGroup($group);
			$data['parent'] = $groupRowId;
			$data['share_type'] = 6;
			foreach($users as $item){
				$data['user'] = $item->getUID();
				$query = $this->connection->prepare('
					INSERT INTO `*PREFIX*share_external_group`
